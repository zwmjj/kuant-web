...
`/audit`, `/docs`, `/ide` and `/source` call `kuant-api` and show what it
returns. `/risk` also calls it, but the risk router builds its portfolio
from a fixed random seed — so the VaR, CVaR and drawdown figures are real
calculations over a synthetic book, and the panel labels them that way.
`/monitor` and `/agents` generate their data in the browser and say so.
`/trading` has no backend in the public API at all and sits disconnected
by design. **No panel in this dashboard reports a real position, a real
fill, or a real broker connection.** The dashboard demonstrates the research
surface; the numbers behind the trading-flavoured panels are synthetic
by construction.
...
| `/risk`       | VaR / CVaR / drawdown / tail risk panel. The risk maths is real (`qf.risk.RiskAnalyzer`, `qf.risk_manager`); the portfolio it runs on is generated by `kuant-api`'s risk router from a fixed seed, which the API reports as `data_source: simulated` and the panel surfaces as "Synthetic portfolio" |
...
| `/trading`    | Paper-trading control surface. **The backend for this panel is not in the public `kuant-api`** — there is no `/api/trading` router there, so the panel reports "disconnected" and shows nothing rather than inventing state. That omission is deliberate: execution code is not published |
